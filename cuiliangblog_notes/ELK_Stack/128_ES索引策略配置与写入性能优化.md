...
> 来源: ELK Stack
> 创建时间: 2024-03-20T23:06:26+08:00
> 更新时间: 2026-01-11T09:27:45.747838+08:00
> 阅读量: 529 | 点赞: 0
...
![](https://via.placeholder.com/800x600?text=Image+8dff7e5669d14eef)
...
![](https://via.placeholder.com/800x600?text=Image+6e1bddc672ff4514)
...
![](https://via.placeholder.com/800x600?text=Image+f427c17b9a446783)

查看页面运行结果，发现已成功去除对应的字段

![](https://via.placeholder.com/800x600?text=Image+1bee4a9535f7ba19)

接下来点击创建并保存管道即可。

![](https://via.placeholder.com/800x600?text=Image+980b0af13e3ca95b)

# 组件模板设置
## 场景需求
我们查看写入的索引信息可以发现，es默认为这个索引设置了1个分片和1个副本，但我们有3个hot节点，我们期望索引可以有3个分片平均分配到hot节点，分摊节点压力，提高查询和写入速度。

![](https://via.placeholder.com/800x600?text=Image+4f9181a57433ff90)
...
![](https://via.placeholder.com/800x600?text=Image+5955e5aac719dbd2)
...
![](https://via.placeholder.com/800x600?text=Image+eba5cda51b5fc3e5)

映射和别名暂不做设置，然后创建组件模板

![](https://via.placeholder.com/800x600?text=Image+2e1cd0e27d1d6aef)

## 配置通用mapping
接下来我们创建一个名为<font style="color:rgb(26, 28, 33);">geoip-mapping的组件模板，只配置mapping相关内容。</font>

![](https://via.placeholder.com/800x600?text=Image+ce5f70cd3ee166fc)
...
![](https://via.placeholder.com/800x600?text=Image+39e25d1bffb10ce3)

接下来创建这个组件模板

![](https://via.placeholder.com/800x600?text=Image+29b179d220ebd4df)
...
![](https://via.placeholder.com/800x600?text=Image+2571f6373ff59d55)

## 创建索引模板
我们找到默认的logs索引模板，查看模板的优先级

![](https://via.placeholder.com/800x600?text=Image+c8bc85e9bdf25957)

接下来创建一个新的索引模板，模板的名称为myapp，索引模式为数据流的名称，并将优先级设置为110，否则会由于优先级低于logs模板而不生效。

![](https://via.placeholder.com/800x600?text=Image+152ce89303c02e80)

在组件模板中我们选择继承先前自定义的模板，完全自定义创建配置。

![](https://via.placeholder.com/800x600?text=Image+e52cb953c24cb979)
...
![](https://via.placeholder.com/800x600?text=Image+f5d5a67285af85c2)
...
![](https://via.placeholder.com/800x600?text=Image+4286c82df57a8e94)
...
![](https://via.placeholder.com/800x600?text=Image+0ccd5d37ba289336)

至此，索引模板创建完成

![](https://via.placeholder.com/800x600?text=Image+1915d7d19221b03c)

## 创建数据流验证
我们删除原本的数据流后，会自动按索引模板设置创建新的数据流。

![](https://via.placeholder.com/800x600?text=Image+92ea727204850103)

查看数据流信息，已关联名为myapp-template的索引模板

![](https://via.placeholder.com/800x600?text=Image+255f72857f79e877)

查看索引设置信息，已按模板设置创建了索引，将分片数调整为3个。

![](https://via.placeholder.com/800x600?text=Image+6b25fe1e5d6709af)
...
![](https://via.placeholder.com/800x600?text=Image+daf77512b50def2c)

## 创建数据流验证
我们还是删除先前的数据流，让es为我们自动创建新的数据流，查看数据流信息验证

![](https://via.placeholder.com/800x600?text=Image+922a12d93f66ab9d)
...
![](https://via.placeholder.com/800x600?text=Image+a31e0ad67b9cf447)
...
![](https://via.placeholder.com/800x600?text=Image+f5fe2e92129e0e20)
...
![](https://via.placeholder.com/800x600?text=Image+222f88d44de53cab)

例如我们使用了<font style="color:rgb(48, 49, 51);">geoip插件获取了用户访问IP的经纬度数据，我们需要指定类型</font>为geo_point

![](https://via.placeholder.com/800x600?text=Image+6225f15b3c0ed218)

## 创建数据流验证
我们还是删除先前的数据流，让es为我们自动创建新的数据流，查看数据流信息验证，可以看到新创建的数据流字段已经按我们的期望设置了数据类型。

![](https://via.placeholder.com/800x600?text=Image+a551cee9762c606a)
...
![](https://via.placeholder.com/800x600?text=Image+978737d36dc2c0eb)

hot阶段配置如下：

![](https://via.placeholder.com/800x600?text=Image+bd1359c4a180b420)

warm阶段配置如下：

![](https://via.placeholder.com/800x600?text=Image+90d73a3b1f9aa629)

cold阶段配置如下：

![](https://via.placeholder.com/800x600?text=Image+a44de689391e067b)

删除阶段配置如下：

![](https://via.placeholder.com/800x600?text=Image+839d8905ff825b75)

## ILM与索引模板关联
创建完ILM策略后，接下来，我们将ILM策略与索引模板关联，这样后续创建的索引都会通过ILM进行管理。

![](https://via.placeholder.com/800x600?text=Image+6df27cbe31614433)
...
![](https://via.placeholder.com/800x600?text=Image+d1fac9cf79a904eb)

hot阶段验证

0-7分钟的数据存放hot节点，且每分钟滚动生成一个索引

![](https://via.placeholder.com/800x600?text=Image+63424881e3fa73b3)

warm阶段验证

7-15分钟数据存放warm节点，合并为1分片，只读索引

![](https://via.placeholder.com/800x600?text=Image+b395ab7ecec1aeb2)

![](https://via.placeholder.com/800x600?text=Image+40a15194fdbeb0a4)

clod验证

15-30分钟数据存放clod节点，调整为0副本，数据降采样

![](https://via.placeholder.com/800x600?text=Image+5a9ff201ba086efd)

![](https://via.placeholder.com/800x600?text=Image+43ad7a5ff20e7a68)

删除验证

超过30分钟以上数据删除。discover只能查到最近30分钟的数据

![](https://via.placeholder.com/800x600?text=Image+14aea560492eddee)
